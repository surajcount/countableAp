...
try {
    ...
    WebDriverWait wait = new WebDriverWait(driver, 30 // Increase wait time to 30 seconds
    )
    ...
    WebElement engbutton = wait.until(ExpectedConditions.elementToBeClickable(By.xpath("//p[contains(text(), '$engClick')]/preceding::td[1]//button")))
...
if(actualstrEL==expectedstrEL){
	println('Engagement letter Loaded')
}else
{
	println('Engagement Letter not loaded')
}


//WebUI.verifyTextPresent('Letter added to the engagement successfully', false)

WebElement comcanML = driver.findElement(By.xpath('//span[contains(text(),\'Management responsibility and acknowledgement\')]'))
...
if(actualstrEL==expectedstrEL){
	println('Management letter Loaded')
}else
{
	println('Management Letter not loaded')
}
