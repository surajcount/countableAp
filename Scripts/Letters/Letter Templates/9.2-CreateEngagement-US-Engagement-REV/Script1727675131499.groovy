...
WebUI.click(findTestObject('CreateEngagement/EngTypeDropDownCompilationREV'))
...
if (WebUI.verifyElementVisible(findTestObject('Object Repository/CreateEngagement/step2Team member'), FailureHandling.OPTIONAL)) {
    ...
    teamMemberUS.click() // Mark test as failed if the team member element is not visible
} else {
...
if (actualstrEL == expectedstrEL) {
    println('US REV Engagement letter Loaded')
} else {
    println('US REV Engagement Letter not loaded')
}

WebUI.delay(5)

WebElement completionsignoffsection = driver.findElement(By.xpath('//span[contains(text(),\'Completion & Signoffs\')]'))

completionsignoffsection.click()




//WebUI.verifyTextPresent('Letter added to the engagement successfully', false)
WebElement comcanML = driver.findElement(By.xpath("//span[contains(text(),'Management representation letter') or contains(text(),'Management Representation letter')]"));

comcanML.click()

String actualstrML = WebUI.getText(findTestObject('Object Repository/Page_UAT (Countable)/div_toaster'))

String expectedstrML = 'Successfully added letter'

System.out.println('Str is ' + actualstrML)

if (actualstrEL == expectedstrEL) {
	println('Management Rep letter Loaded')
} else {
	println('Management Rep Letter not loaded')
}

// Verify the toaster message
//WebUI.verifyTextPresent('Successfully added letter', false)
WebUI.delay(5)
